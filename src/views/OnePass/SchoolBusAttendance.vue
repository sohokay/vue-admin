<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      >
        <el-option
          v-for="item in listFilter.schoolBus"
          :key="item.id"
          :label="item.name"
          :value="item.id"
        />
      </el-select>
      <el-select v-model="listQuery.upOrDown" placeholder="上下车" clearable style="width: 120px" class="filter-item">
        <el-option
          v-for="item in listFilter.upOrDown"
          :key="item.id"
          :label="item.name"
          :value="item.id"
        />
    ...
    >
      ...
      <el-table-column label="学号" min-width="100px" align="center">
        <template slot-scope="{row}">
          <!--          <span class="link-type" @click="handleUpdate(row)">{{ row.studentNumber }}</span>-->
          <span>{{ row.studentNumber }}</span>
          <!--          <el-tag>{{ row.studentNumber | typeFilter }}</el-tag>-->
        </template>
      </el-table-column>
      <el-table-column label="卡号" min-width="150px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="刷卡时间" align="center" min-width="95">
        <template slot-scope="{row}">
          <span>{{ row.clockIn |parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="考勤状态" class-name="status-col" min-width="100">
        <template slot-scope="{row}">
      ...
      <el-table-column label="进出校" class-name="status-col" min-width="100">
        <template slot-scope="{row}">
          <el-tag :type="row.inOrOut | statusFilter">
            {{ row.inOrOut === 1 ? '进校' : '出校' }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="主机ID" class-name="status-col" min-width="100">
        <template slot-scope="{row}">
          <span> {{ row.deviceCode }}</span>
          <!--          <el-tag :type="row.deviceCode | statusFilter">-->
          <!--            {{ row.deviceCode }}-->
          <!--          </el-tag>-->
        </template>
      </el-table-column>
